...
def update_note(note):
    notes_name = {'Имя', 'Заголовок', 'Описание', 'Статус'}
    while True:
        note_number = int(input('Введите номер заметки которой хотите изменить: '))-1
        field = input('Какие данные вы хотите обновить? (Имя, заголовок, описание, статус): ')
        if field in notes_name:
            break
        else:
    ...
    note[note_number][field] = new_value
...
def delete_note():
    while True:
        ...
        if title_notes:
            ...
            print(Back.RED, f"Заметка с заголовком {criterion} успешно удалена.", Style.RESET_ALL)
...
while True:
    print('\n', Back.LIGHTMAGENTA_EX, Fore.BLACK, Style.BRIGHT, 'Добро пожаловать в "Менеджер заметок"', Style.RESET_ALL)
    print(Back.MAGENTA, Fore.BLACK,'1: Отображение списка заметок.', Style.RESET_ALL)
    print(Back.MAGENTA, Fore.BLACK,'2: Создание новой заметки.', Style.RESET_ALL)
    print(Back.MAGENTA, Fore.BLACK,'3: Изменение заметок', Style.RESET_ALL)
    print(Back.MAGENTA, Fore.BLACK,'4: Удаление заметок', Style.RESET_ALL)
    print(Back.MAGENTA, Fore.BLACK,'5: Поиск заметки.', Style.RESET_ALL)
    print(Back.MAGENTA, Fore.BLACK,'6: Завершение программы.', Style.RESET_ALL)
